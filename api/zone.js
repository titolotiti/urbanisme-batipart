export default async function handler(req, res) {
  ...
  function pickReglement(docs) {
    ...
    return (
      ...
      docs.find(d => u(d).endsWith('.pdf') && !u(d).match(/graphique|rapport|padd|oap|procedure/i))
    );
  }
  ...
  try {
    ...
    // Normalisation arrondissements → code commune
    ...
    // ─── 3. Trouver l'URL du règlement ───
    let pluUrl = null, pluName = null;

    // ══ SOURCE 1 : WFS GPU — wfs_du:doc_urba ══
    // Base officielle IGN, couvre TOUTES les communes de France
    // Filtre par partition (le plus précis) puis par codcom en fallback
    const wfsBase = 'https://data.geopf.fr/wfs/ows?SERVICE=WFS&VERSION=2.0.0&REQUEST=GetFeature&TYPENAMES=wfs_du:doc_urba&OUTPUTFORMAT=application/json&COUNT=20&sortBy=datval+D';

    // Essai 1a : filter par partition (exact)
    if (!pluUrl && partition) {
      try {
        const wR = await fetch(
          `${wfsBase}&CQL_FILTER=partition='${encodeURIComponent(partition)}'`,
          { headers: H }
        );
        ...
        if (wD.features?.length) {
          ...
          const url = reg ? (reg.href || reg.url) : null;
          if (url) {
            pluUrl = url;
            pluName = (reg.libelle || 'Règlement PLU') + fmtDate(url);
            console.log('✓ WFS partition:', pluUrl);
          }
        }
      } catch(e) { console.log('WFS partition err:', e.message); }
    }

    // Essai 1b : filter par codcom (si partition n'a pas marché)
    if (!pluUrl) {
      try {
        const wR = await fetch(
          `${wfsBase}&CQL_FILTER=codcom='${citycode}'`,
          { headers: H }
        );
        const wD = await wR.json();
        if (wD.features?.length) {
          ...
          if (url) {
            pluUrl = url;
            pluName = (reg.libelle || 'Règlement PLU') + fmtDate(url);
            console.log('✓ WFS codcom:', pluUrl);
          }
        }
      } catch(e) { console.log('WFS codcom err:', e.message); }
    }

    // ══ SOURCE 2 : APICarto document API ══
    if (!pluUrl && partition) {
      try {
    ...
    // ══ SOURCE 3 : Base de données minimale (dernier recours) ══
    // Uniquement pour communes dont les APIs ci-dessus échouent
    if (!pluUrl) {
